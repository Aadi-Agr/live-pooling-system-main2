...
const StudentPollPage = () => {
  ...
  const handleSubmit = () => {
    if (selectedOption) {
      ...
      if (username) {
        console.log("📤 STUDENT Submitting answer:", { username, option: selectedOption, pollId });
  ...
  useEffect(() => {
    socket.on("pollCreated", (pollData) => {
      console.log("🎯 STUDENT received pollCreated event:", pollData);
    ...
    socket.on("pollResults", (updatedVotes) => {
      console.log("📊 STUDENT received pollResults:", updatedVotes);
