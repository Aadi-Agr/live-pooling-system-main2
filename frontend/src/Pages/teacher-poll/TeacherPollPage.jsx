...
let apiUrl = process.env.REACT_APP_API_BASE_URL || "http://localhost:5000";

const socket = io(apiUrl);

// Debug: Socket connection status
socket.on("connect", () => {
  console.log("🟢 TEACHER POLL Socket connected:", socket.id);
});

socket.on("disconnect", () => {
  console.log("🔴 TEACHER POLL Socket disconnected");
});

socket.on("connect_error", (error) => {
  console.error("❌ TEACHER POLL Connection error:", error);
});
...
const TeacherPollPage = () => {
  ...
  useEffect(() => {
    socket.on("pollCreated", (pollData) => {
      console.log("🎯 TEACHER POLL received pollCreated:", pollData);
    ...
    socket.on("pollResults", (updatedVotes) => {
      console.log("📊 TEACHER POLL received pollResults:", updatedVotes);
